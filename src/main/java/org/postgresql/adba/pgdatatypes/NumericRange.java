...
public class NumericRange implements Comparable<NumericRange> {
   ...
   */
  public NumericRange(BigDecimal lower, BigDecimal upper, boolean lowerInclusive, boolean upperInclusive) {
    this.lower = lower;
    this.upper = upper;
  ...
  @Override
  public int compareTo(NumericRange lr) {
    if (empty && lr.empty) {
      return 0;
    }

    int c = Objects.compare(lr.lower, lower, BigDecimal::compareTo);

    if (c != 0) {
      return c;
    }

    c = Objects.compare(lr.upper, upper, BigDecimal::compareTo);
