...
public class IntegerRange implements Comparable<IntegerRange> {
   ...
   */
  public IntegerRange(Integer lower, Integer upper, boolean lowerInclusive, boolean upperInclusive) {
    this.lower = lower;
    this.upper = upper;
  ...
  public boolean equals(Object o) {
    ...
    if (empty && integerRange.empty) {
      return true;
    }
    return lowerInclusive == integerRange.lowerInclusive
        && upperInclusive == integerRange.upperInclusive
        && Objects.equals(lower, integerRange.lower)
        && Objects.equals(upper, integerRange.upper);
  }
  ...
  @Override
  public int compareTo(IntegerRange lr) {
    if (empty && lr.empty) {
      return 0;
    }

    int c = Integer.compare(lr.lower, lower);

    if (c != 0) {
      return c;
    }

    c = Integer.compare(lr.upper, upper);
  ...
  public String toString() {
    ...
    return (lowerInclusive ? "[" : "(")
        + (lower == null ? "" : Integer.toString(lower)) + ","
        + (upper == null ? "" : Integer.toString(upper))
        + (upperInclusive ? "]" : ")");
  }
}
